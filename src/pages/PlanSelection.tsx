...
const PlanSelection = () => {
  const plans = [{
    ...
    popular: false,
    features: ['Endereço Comercial para divulgação', 'Endereço Fiscal para abertura/alteração de CNPJ', 'Recebimento de Correspondências', 'Atendimento Telefônico', 'Inscrição Estadual', 'Equipe para receber fiscalizações'],
    bonuses: []
  }, {
    name: '1 ANO',
    price: 'R$ 99',
    ...
    popular: true,
    features: ['Endereço Comercial para divulgação', 'Endereço Fiscal para abertura/alteração de CNPJ', 'Recebimento de Correspondências', 'Atendimento Telefônico', 'Inscrição Estadual', 'Equipe para receber fiscalizações'],
    bonuses: ['5% de desconto em Salas de Reunião', '2h/mês em Salas de Reunião']
  }, {
    name: '2 ANOS',
    price: 'R$ 69',
    period: '/mês',
    ...
    popular: false,
    features: ['Endereço Comercial para divulgação', 'Endereço Fiscal para abertura/alteração de CNPJ', 'Recebimento de Correspondências', 'Atendimento Telefônico', 'Inscrição Estadual', 'Equipe para receber fiscalizações'],
    bonuses: ['5% de desconto em Salas de Reunião', '2h/mês em Salas de Reunião', '1h de Consultoria de Marketing Digital']
  }];
  return <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {plans.map((plan, index) => <Card key={index} className={`relative ${plan.popular ? 'ring-2 ring-on-lime scale-105' : ''} on-card h-full`}>
                ...
                <CardHeader className="text-center pb-8">
                  ...
                  <div className="space-y-2">
                    ...
                    {plan.annualPrice && <p className="text-sm text-gray-600">ou {plan.annualPrice}</p>}
                    {plan.setupFee && <p className="text-sm text-red-600 font-medium">+ {plan.setupFee}</p>}
                  </div>
                </CardHeader>
                ...
                <CardContent className="space-y-6">
                  ...
                  {plan.bonuses.length > 0 && <div>
                      <h4 className="font-semibold text-on-lime mb-3">Bônus:</h4>
                      <ul className="space-y-2">
                        {plan.bonuses.map((bonus, i) => <li key={i} className="flex items-start gap-2">
                            <Check className="w-5 h-5 text-on-lime mt-0.5 flex-shrink-0" />
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-gray-600 mb-8">Processamos mais de 30 correspondências diárias com total segurança e agilidade</p>
