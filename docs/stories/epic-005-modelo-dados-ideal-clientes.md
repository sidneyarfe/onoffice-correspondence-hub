...
| 5.2 | **Cobrança:** evoluir `pagamentos → faturas` (status/origem/vencimento/centavos) + **job diário** marca vencidas + derivação na leitura + backfill | DB + Backend | @data-engineer + @dev | 5.1 | Médio-Alto · 🟡 **migração pronta (aguardando push)** |
...
| 5.4 | **Comércio:** rename `cliente_planos → assinaturas` (ALTER + VIEW compat); `pedidos`/`pedido_itens` p/ avulsos; multi-produto por cliente; aba Financeiro lista assinaturas + avulsos | DB + UI | @data-engineer + @dev | 5.1, 5.2 | Alto |
